...
async def create_db(self: commands.AutoShardedBot):     
  await self.db.execute("CREATE TABLE IF NOT EXISTS reskin_toggle (guild_id bigint PRIMARY KEY)")
  ...
  await self.db.execute("CREATE TABLE IF NOT EXISTS flags_scores (guild_id BIGINT NOT NULL, user_id BIGINT NOT NULL, round_num INT NOT NULL, points INT DEFAULT 0, PRIMARY KEY (guild_id, user_id, round_num));")
  await self.db.execute("CREATE TABLE IF NOT EXISTS vape_data (user_id BIGINT PRIMARY KEY, flavor VARCHAR(255) NOT NULL, vape_count INT NOT NULL DEFAULT 0)")
  await self.db.execute("CREATE TABLE IF NOT EXISTS vape_counts (user_id BIGINT, flavor VARCHAR(255), count INT NOT NULL DEFAULT 0, PRIMARY KEY (user_id, flavor), FOREIGN KEY (user_id) REFERENCES vape_users(user_id))")
